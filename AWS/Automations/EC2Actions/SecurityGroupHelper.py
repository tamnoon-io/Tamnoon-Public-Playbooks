    ...
    for lambda_asset in response_lambda:
        if (
            ...
            for group in lambda_asset["VpcConfig"]["SecurityGroupIds"]:
                ...
                check_security_group_usage(
                    asset_ids, group, {"type": "Lambda", "id": lambda_id}, sg_usage
                )
...
def get_ingress_rules_for_sg(
    ...
    if IpProt:
        routput = list(filter(lambda x: x.get("IpProtocol") == IpProt, routput))
    if Ports:  # we're only matching single-port rules
        ...
        for port in Ports:
            elist.extend(
                list(
                    filter(
                        lambda x: x.get("ToPort") == port and x.get("FromPort") == port,
                        routput,
                    )
                )
            )
...
def replace_sg_rule(
    ...
    try:
        response = ec2client.authorize_security_group_ingress(
            GroupId=rule["GroupId"], IpPermissions=[new_rule], DryRun=is_dry_run
        )
        ...
        if "aready exists" in str(e) or "InvalidPermission.Duplicate" in str(
            ...
            sgrids.append(message)
    ruleremoved = remove_rules_from_sg(ec2client, rule, is_dry_run)["ruleremoved"]
    return {
        "inputrule": rule,
